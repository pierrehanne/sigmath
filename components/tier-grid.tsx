        ...
        return (
          <article className={`tier-card tier-card--${tier.tone}`} key={tier.slug} style={{ "--tier-color": tier.color } as React.CSSProperties}>
            <div className="tier-card-topline">
              ...
              <span>{tier.age} ans</span>
            </div>
            <div className="tier-icon" aria-hidden="true"><Icon strokeWidth={1.5} /></div>
            <div className="tier-card-content">
              <h3>{tier.title}</h3>
              <p>{tier.description}</p>
              <div className="subject-list" role="list" aria-label="Matières">
                {tier.subjects.map((subject) => <span role="listitem" key={subject}>{subject}</span>)}
              </div>
            </div>
            <Link href={`/${locale}/${tier.slug}`} className="tier-link" aria-label={`${t.visit}: ${tier.title}`}>
