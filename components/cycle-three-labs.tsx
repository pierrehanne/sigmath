...
export function FractionLineLab() {
...
export function DecimalGridLab() {
  const [hundredths, setHundredths] = useState(137);
  const t = labText;
  const whole = Math.floor(hundredths / 100);
  const part = hundredths % 100;
  const decimal = (hundredths / 100).toFixed(2).replace(".", ",");
  return (
    <section className="cycle3-lab cycle3-lab--decimal" aria-label={t.decimalTitle}>
      ...
      <div className="decimal-stage">
        <div className="decimal-grids" aria-hidden="true">{[0, 1, 2].map((unit) => <div key={unit}><div className="hundred-grid">{Array.from({ length: 100 }, (_, index) => <i className={unit * 100 + index < hundredths ? "filled" : ""} key={index} />)}</div><small>1 unité</small></div>)}</div>
...
export function BalanceLab() {
  ...
  return (
    <section className="cycle3-lab cycle3-lab--balance" aria-label={t.balanceTitle}>
      ...
      <div className="equation-tabs" role="group" aria-label="Choisir une égalité">{equations.map((item, index) => <button type="button" aria-pressed={equationIndex === index} className={equationIndex === index ? "active" : ""} onClick={() => { setEquationIndex(index); setGuess(5); }} key={item.target}>{item.factor === 1 ? "□" : `${item.factor} × □`}{item.add ? ` + ${item.add}` : ""} = {item.target}</button>)}</div>
    ...
    <section className="cycle3-lab cycle3-lab--area" aria-label={t.areaTitle}>
      ...
      <div className="area-stage">
        <svg className="area-unit-grid" viewBox="0 0 280 230" role="img" aria-label={`${width} × ${height} = ${width * height} cm²`}>
          ...
          <text x={30 + width * 11} y={17} textAnchor="middle">{width} cm</text><text x={30 + width * 11} y={height * 22 + 48} textAnchor="middle">1 case = 1 cm²</text>
        </svg>
...
export function AngleLab() {
  ...
  return (
    <section className="cycle3-lab cycle3-lab--angle" aria-label={t.angleTitle}>
      ...
      <input aria-label="Mesure de l’angle" type="range" min="0" max="180" step="5" value={angle} onChange={(event) => setAngle(Number(event.target.value))} />
    </section>
  );
}
  ...
  return (
    <section className="cycle3-lab cycle3-lab--algorithm" aria-label={t.algorithmTitle}>
      ...
      <div className="algorithm-stage">
        <div><div className="robot-grid" role="img" aria-label={`Colonne ${robot.x + 1}, ligne ${robot.y + 1}, ${directions[robot.direction]}`}>{Array.from({ length: 16 }, (_, index) => <i key={index} className={visited.includes(index) ? "visited" : ""}>{index === robot.y * 4 + robot.x ? <ArrowUp style={{ transform: `rotate(${robot.direction * 90}deg)` }} /> : null}</i>)}</div><p className="robot-status" role="status">{robot.blocked ? "Un mur ! Modifie le programme ou recommence." : `${cursor}/${program.length} instructions exécutées`}</p></div>
        <div className="program-panel"><strong>{t.program} · {program.length}/10</strong><ol className="robot-instructions">{program.map((instruction, index) => <li key={index} aria-current={index === cursor - 1 ? "step" : undefined}>{instructionLabel(instruction)}</li>)}</ol>{program.length === 0 ? <p>{t.empty}</p> : null}
          ...
          {program.length === 10 ? <p role="status">10 instructions maximum. Retire la dernière pour modifier.</p> : null}
          <button type="button" disabled={cursor >= program.length || robot.blocked} onClick={step}><StepForward />Un pas</button>
          <button type="button" className="run-program" disabled={!program.length} onClick={run}><Play />{t.run}</button>
          <button type="button" disabled={!program.length} onClick={() => { setProgram((current) => current.slice(0, -1)); restart(); }}>Retirer la dernière</button>
